  ...
  useEffect(() => {
    const loadRestaurants = async () => {
      try {
        ...
        if (data.length > 0) {
          // Safely extract city and cuisine values
          const extractCities = data
            .map((r) => r.city || "Unknown")
            .filter(Boolean);
          const extractCuisines = data
            .map((r) => r.cuisine || "Unknown")
            .filter(Boolean);

          const uniqueCities = [...new Set(extractCities)];
          const uniqueCuisines = [...new Set(extractCuisines)];
  ...
  const filteredRestaurants = Array.isArray(restaurants)
    ? restaurants
        .filter((restaurant) => {
          ...
          return nameMatch && cuisineMatch && ratingMatch && cityMatch;
        })
        .sort((a, b) => {
          let comparison = 0;
          ...
          <table>
            <thead>
              <tr>
                ...
                <th onClick={() => handleSortChange("cuisine")}>
                  Cuisine
                  {sortBy === "cuisine" && (
                    <span className="sort-indicator">
                      {sortDirection === "asc" ? "↑" : "↓"}
