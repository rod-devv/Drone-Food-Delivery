...
const DeliveryMap = ({
  ...
  // Update drone positions every second
  // Inside the drone position update interval effect in DeliveryMap.jsx
  useEffect(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      // Update each drone marker
      deliveries.forEach((delivery) => {
        if (delivery.status === "on-the-way" && delivery.droneDelivery) {
          ...
          if (isComplete) {
            // Remove the drone if journey is complete
            const droneMarker = markers.current.drones[delivery._id];
            if (droneMarker) {
              droneMarker.remove();
              delete markers.current.drones[delivery._id];
              console.log(
                `Removed completed drone for delivery ${delivery._id}`
              );
            }
          } else {
            // Only update or create drone if journey is still active
            activeDroneCount++;
      ...
      // If no active drones, clear the interval to save resources
      if (activeDroneCount === 0) {
        clearInterval(intervalRef.current);
        intervalRef.current = null;
        console.log("No active drones - stopping interval");
      }
