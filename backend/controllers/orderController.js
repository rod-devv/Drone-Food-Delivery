...
const getOrdersByCity = async (req, res) => {
  try {
    ...
    // Look for city in the order document since you're saving it there
    const orders = await Order.find({
      city: city.toLowerCase(),
    })
      .populate("restaurant")
      // Remove the customer populate that's causing the error
      // .populate("customer")
      .populate("user", "name email"); // Use user instead if you need customer info
